...
<div class="container">
	<form action="{{route('products.update',$product->id)}}" class="row" method="POST" enctype="multipart/form-data" id="update_product">
		@csrf
		<div class="bg-white p-5 col-xl-8 col-lg-8 col-md-8 col-sm-12 shadow-lg">
			...
			<div class="form-group">
				...
				<input type="text"  class="form-control" value="{{__($product->name)}}" name="name" required>
			</div>
			<div class="form-group">
			...
			<div class="form-group" id="condition-show" <?= ($product->condition) ? '' : 'style="display: none"';  ?> > 
				...
				<select name="condition" class="form-control">
					...
					<option value="new" <?= ($product->condition == 'new') ? 'selected="selected"' : '' ?> >{{__('New')}}</option>
					<option value="used" <?= ($product->condition == 'used') ? 'selected="selected"' : '' ?> >{{__('Used')}}</option>
				</select>
			</div>
			<div class="form-group">
				...
				<select name="category" id="category" class="form-control category" required>
					...
					@foreach($categories as $category)
						<option value="{{$category->id}}" <?= ($category->id == $product->category_id ) ? 'selected="selected"' : '' ?> data-url="{{route('admin.categorysubcategory.options',$category->id)}}">{{__($category->name)}}</option>
					@endforeach
				</select>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<select name="childcategory" id="childcategory" class="form-control childcategory" <?=$product->subcategory_id == null ? "disabled":""?> >
					...
					@if($product->subcategory_id != null)
						...
						@else
						@foreach($product->subcategory->childs as $child)
							<option value="{{$child->id}}" <?= ($child->id == $product->childcategory_id ) ? 'selected="selected"' : '' ?> data-url="{{route('admin.childcategory.attributes',$child->id)}}" >{{__($child->name)}}</option>
						@endforeach
						@endif
					@endif
				</select>
					...
					<div id="category_attributes">
						...
						@if (!empty($catAttributes))
							@foreach ($catAttributes as $catAttribute)
								<div class="row pt-4">
									<div class="col-md-3">
										<label>{{__(ucwords($catAttribute->name))}}*</label>
									</div>
									 <div class="col-md-9">
										 @php
										 	$i = 0;
										 ...
										 @foreach ($catAttribute->attribute_options as $optionKey => $option)
										 	...
										 	<div class="custom-control custom-checkbox custom-control-inline">
												 ...
												 @endif
												 >
												 <label class="custom-control-label" for="{{ $catAttribute->input_name }}{{$option->id}}">{{ __($option->name) }}</label>
											</div>
					...
					<div id="subcategory_attributes">
						...
						@if (!empty($subAttributes))
							@foreach ($subAttributes as $subAttribute)
								<div class="row pt-4">
									<div class="col-md-3">
										<label>{{__(ucwords($subAttribute->name))}}*</label>
									</div>
									 <div class="col-md-9">
											 @php
											 	$i = 0;
											 ...
											 @foreach ($subAttribute->attribute_options as $option)
												 ...
												 @endphp
											       ...
											       <div class="custom-control custom-checkbox custom-control-inline">
											          ...
											          @endif
											          >
											          <label class="custom-control-label" for="{{ $subAttribute->input_name }}{{$option->id}}">{{ __($option->name) }}</label>
											       </div>
												 @php
													 if ($checked == 1) {
													 	$i++;
					...
					<div id="childcategory_attributes">
						...
						@if (!empty($childAttributes))
							@foreach ($childAttributes as $childAttribute)
								<div class="row pt-4">
									<div class="col-md-3">
										<label>{{__(ucwords($childAttribute->name))}}*</label>
									</div>
									 <div class="col-md-9">
										 @php
										 	$i = 0;
										 ...
										 @foreach ($childAttribute->attribute_options as $optionKey => $option)
											 ...
											 <div class="custom-control custom-checkbox custom-control-inline">
													 ...
													 @endif
													 >
													 <label class="custom-control-label" for="{{ $childAttribute->input_name }}{{$option->id}}">{{ __($option->name) }}</label>
												</div>
											 @php
												 if ($checked == 1) {
												 	$i++;
			...
			<div class="form-group" id="size-show" <?= (count($product->sizes) > 0 ) ? '' : 'style="display: none"';  ?> >
				...
				<div class="row py-3 offset-1 extra-sizes">
					<div class="form-group col-4 pb-3">
						...
						<input id="size" placeholder="eg: M" type="text" name="sizes[]" value="{{__($p_size->title)}}" class="form-control size-inputs">
					</div>
					<div class="form-group col-4 pb-3">
			...
			<div class="form-group" id="color-show" <?= (count($product->colors) > 0 ) ? '' : 'style="display: none"';  ?> >
				...
				<div class="row py-3 offset-1 extra-colors">
					...
					<input id="color_name" placeholder="eg: Black" value="{{__($p_color->name)}}" type="text" name="colors_name[]" class="form-control color-inputs">
					</div>
					<div class="form-group col-4">
					<label>{{__('Color*')}}
							<span style="display: block;font-size: 10px; font-weight: 400">
		...
		<div class="bg-white  shadow-lg col-xl-3 col-lg-3 col-md-12 col-sm-12  offset-lg-1">
				...
				<div class="form-group">
					...
					<div >
						<button type="button" class="btn rounded-0 btn-primary" data-toggle="modal" data-target="#images-modal"><i class="mdi mdi-library-plus"></i> {{__('Change Images')}}</button>
					</div>
		...
		<div class="bg-white p-5 col-xl-12 col-lg-8 col-md-8 col-sm-12 shadow-lg" style="margin-top: 1%">
			...
			<div id="seo-show" <?= ($product->seo) ? '' : 'style="display: none;"' ?> >
				...
				<div class="form-group">
					...
					<textarea rows="10" class="form-control" class="seo-inputs" name="meta_description">{{($product->seo) ? $product->seo->meta_description : ''}}</textarea>
				</div>
			</div>
			<div class="form-group pt-5">
				<button type="submit" class="btn btn-primary btn-lg float-right" id="submit-btn"> {{__('UPDATE')}}</button>
			</div>
		</div>
	</form>
</div>
